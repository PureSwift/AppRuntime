...
| `capabilities` | array of strings | no | Requested capabilities. Defined values: `Display`, `Audio`, `Network`. Absent array = no capabilities. |
...
`arm64`, `armv7`, `armv6`, `armv5`, `x86_64`, `x86`.

Mapping from `uname -m`: `aarch64`/`arm64` → `arm64`; `x86_64`/`amd64` →
`x86_64`; `i386`–`i686` → `x86`; `armv7*` → `armv7`; `armv6*` → `armv6`;
`armv5*` → `armv5`.
...
|---|---|
| `arm64` | `arm64` → `armv7` (needs AArch32) → `x86_64` via box64 (needs box64) → `x86` via box86 (needs box86 **and** AArch32) |
| `armv7` | `armv7` → `x86` via box86 (needs box86) |
| `x86_64` | `x86_64` → `x86` (needs multilib) |
